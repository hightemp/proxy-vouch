...
async def main():
    ...
    with tempfile.TemporaryDirectory(prefix="proxy-vouch-native-") as directory:
        ...
        try:
            ...
            await driver.js("const checkbox=[...document.querySelectorAll('.checkbox-label')].find(el=>el.textContent.includes('Check download speed and transfer limits')).querySelector('input'); if(checkbox.checked) checkbox.click();")
            ...
            settings = dict(url=f"http://127.0.0.1:{target}/slow",fallbackUrl="",ipEcho=True,countryLookup=False,anonymityCheck=False,speedCheck=False,expectedStatus=200,bodyContains="",concurrency=2,rateLimit=100,connectTimeoutMs=1000,attemptTimeoutMs=8000,totalTimeoutMs=15000,retries=0)
